...
#define EXPORT_TIME 1000000 //Export flow after this many seconds
...
int processPacket( char *packet, metaData_t *meta, void *flowdata )
{
  ...
  /* If the flow is greater than 1 second long, ignore all further input */
  diff = now - data->first;
  if ((diff > EXPORT_TIME) && (data->first > 0)){
	data->time_cap = 0;
  }
