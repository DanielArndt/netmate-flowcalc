...
/*! \file  netai_flowstats.c
    ...
    $Id: netai_flowstats.c 100 2009-07-14 07:55:43Z szander $
*/
...
typeInfo_t exportInfo[] = { { UINT32, "total_fpackets"    }, // totals
...
struct flowData_t {
  unsigned long fpackets;
  unsigned long fbytes;
  unsigned long bpackets;
  unsigned long bbytes;

  unsigned short min_fpktl;
  double pktl_fsum;
  double pktl_fsqsum;
  unsigned short max_fpktl;
  unsigned short min_bpktl;
  double pktl_bsum;
  double pktl_bsqsum;
  unsigned short max_bpktl;
  
  unsigned long min_fiat;
  unsigned long max_fiat;
  double iat_fsum;
  double iat_fsqsum;
  unsigned long min_biat;
  unsigned long max_biat;
  double iat_bsum;
  double iat_bsqsum;

  unsigned long long flast;
  unsigned long long blast;
  // first packet for the flow
  unsigned long long first;
  // last start of active period
  unsigned long long lactive_start;

  // no of active periods
  unsigned short activep;
  // active time
  double activet;
  unsigned long long min_active;
  unsigned long long max_active;
  double active_sqsum;

  // no of idle periods
  unsigned short idlep;
  double idlet;
  unsigned long long min_idle;
  unsigned long long max_idle;
  double idle_sqsum;

  // number of packet pairs (number of iat samples)
  unsigned long fivals;
  unsigned long bivals;
  ...
  unsigned long fpsh_cnt, bpsh_cnt; 
  unsigned long furg_cnt, burg_cnt; 

  unsigned long long idle_threshold;
  unsigned long long min_duration;
  unsigned long long time_cap;
};
...
int processPacket( char *packet, metaData_t *meta, void *flowdata )
{
    ...
    len = ntohs(*((unsigned short *) &meta->payload[meta->offs[1] + 2]));
    ...
    proto = (unsigned short) meta->payload[meta->offs[1] + 9];

    /* set valid immediatly for ICMP */
    if (!data->valid && (proto == 1)) {
      data->valid++;
    }

    /* set valid if UDP and at least 1 byte payload */
    if (!data->valid && (proto==17)) {
      /* must at least have one byte of data */
      unsigned short udplen = ntohs(*((unsigned short *) &meta->payload[meta->offs[2] + 4]));
      if (udplen > 8) { 
        data->valid++;
      }
    }
    ...
    if (proto==6) {
      unsigned short flags = (unsigned short) meta->payload[meta->offs[2] + 13];
      ...
      if (!data->valid) {
	if ( (data->cstate == STATE_ESTABLISHED) || (data->sstate == STATE_ESTABLISHED) /*||
	     isSet(TCP_ACK, flags) */) {
	  /* if IP packet is longer than IP/TCP headers than there must be data */
	  unsigned short iphlen = (meta->payload[meta->offs[1]] & 0x0F) * 4;
	  unsigned short tcphlen = ((meta->payload[meta->offs[2] + 12] & 0xF0) >> 4) * 4;
...
int resetFlowRec( void *flowdata )
{
    ...
    unsigned long long idle_threshold = data->idle_threshold;
    unsigned long long min_duration = data->min_duration;
    unsigned long long time_cap = data->time_cap;

    memset(data, 0, sizeof(struct flowData_t));

    data->idle_threshold = idle_threshold;
    data->min_duration = min_duration;
    data->time_cap = time_cap;
...
int exportData( void **exp, int *len, void *flowdata )
{
    ...
    /* add the last active period (so far) to the exported stats (but not the flow data!) */
    diff = getLast(data) - data->lactive_start;
    unsigned long long activet = data->activet + diff;
    unsigned long long max_active = data->max_active;
    if (diff > max_active) {
      max_active = diff;
    }
    unsigned long long min_active = data->min_active;
    if ((diff < min_active) || (min_active == 0ULL)) {
      min_active = diff;
    }
    unsigned long long active_sqsum = data->active_sqsum + diff*diff;

    if ((data->fpackets > 1) || (data->bpackets > 1)) {
      assert(data->activep > 0);
    }

    ADD_UINT64( min_active );
    ADD_UINT64( (unsigned long long) (activet/data->activep) );
    ADD_UINT64( max_active );
    ADD_UINT64( (data->activep < 2) ? 0LL : (unsigned long long) stddev(active_sqsum, activet, data->activep) );
...
char* getModuleInfo(int i)
{
    ...
    case I_VERSION:   return "1.0";
    case I_CREATED:   return "2004/06/08";
    case I_MODIFIED:  return "2006/01/23";
    case I_BRIEF:     return "compute various flow statistics";
    case I_VERBOSE:   return "this module computes various statistics based on packet lenghts"
