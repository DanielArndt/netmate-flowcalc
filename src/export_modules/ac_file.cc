...
Description:
export into ASCII file(s) suitable for importing to autoclass 

$Id: ac_file.cc 748 2009-09-10 02:54:03Z szander $
...
// module global variables (common for all rules using the ac_file export module)
...
{
    exportDir = confMan->getValue("ExportDir", "EXPORTER", "ac_file");
    ...
    exportFilename = confMan->getValue("ExportFilename", "EXPORTER", "ac_file");
...
int timeout( int id )
{
    // called by netmate if module has timers defined - in ac_file module unused!
    return 0;
}
...
#ifdef DEBUG
    cerr << "ac_file::exportData (rule='" << frec->getRuleName() << "')" << endl;
#endif
