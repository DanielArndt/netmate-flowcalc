...
$Id: text_file.cc 748 2009-09-10 02:54:03Z szander $
...
// module global variables (common for all rules using the text_file export module)

static string exportDir;
static string exportFilename;

// task specific variables (one such record is stored by netmate per task
//                          using the text_file export module)
...
{
    exportDir = confMan->getValue("ExportDir", "EXPORTER", "text_file");
    ...
    exportFilename = confMan->getValue("ExportFilename", "EXPORTER", "text_file");
                             ...
                             ofstream &ofile, int expFlowId, int final, int expFlowStatus )
{
    ...
    while ((nrows = mdata->getNextList()) > -1) {
        ...
        ofile << "task: " << taskName 
              << ", module: " << mdata->getModName() 
              << ", tstamp: " << tstamp << "." << tval.tv_usec
              << ", rows: " << nrows
              << ", columns: " << mdata->getFlowKeyNum() + mdata->getFlowDataColNum() + expFlowId
              << ", colnames:";
...
int exportData( FlowRecord *frec, void *expData ) 
{
...
#ifdef DEBUG
    cerr << "text_file::exportData (rule='" << frec->getRuleName() << "')" << endl;
#endif
...
#ifdef PROFILING
    {
        ...
        cerr << "text_file::exportData took "
